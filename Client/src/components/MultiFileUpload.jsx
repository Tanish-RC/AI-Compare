...
export default function MultiFileUpload({
  ...
  const uploadOnly = async () => {
    ...
    try {
      for (const file of files) {
        ...
        if (!createRes.ok) {
          throw new Error("Failed to create chart entry");
        }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)] bg-white rounded-lg shadow-md">
      ...
      <div className="flex justify-end space-x-3 p-4 border-t bg-white sticky bottom-0">
        ...
        <button
          ...
          onClick={uploadOnly}
          disabled={isUploading || files.length === 0}
          className="inline-flex items-center px-4 py-2 bg-green-600 hover:bg-green-700 rounded-md shadow-lg border border-green-700 font-medium"
        >
          {isUploading ? (
            <>
              <Loader2 className="animate-spin -ml-1 mr-2 h-4 w-4" />
